...
const Card = ({user}) => {
  ...
  <div className=" p-1 h-78 overflow-hidden w-60 bg-amber-300 border border-black rounded-xl flex flex-col gap-1">
      <div className=" h-40 overflow-hidden object-cover rounded-t-xl w-full bg-red-500">
          <img className="h-full " src={user.Image} alt="not found"  />
      </div>
    ...
    <button className="py-2 px-5 text-white rounded-2xl active:translate-y-[-2px] bg-blue-500">Update</button>
    <button className="py-2 px-5 text-white rounded-2xl active:translate-y-[-2px] bg-red-500">Delete</button>
      </div>
  </div>
  )
